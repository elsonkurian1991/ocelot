...
public class CFGVisitor_new_to_check extends ASTVisitor {
    private CFG graph;
    private Map<IASTNode, CFGNode> astToCfgMap;
     ...
     */
    public CFGVisitor_new_to_check(CFG pGraph, String pFunctionName) {
        this.graph = pGraph;
        this.functionName = pFunctionName;

        this.astToCfgMap = new HashMap<>();
    ...
    public int visit(IASTDeclaration name) {
        ...
        if (name instanceof CASTFunctionDefinition) {
            ...
            // Re-route outputs that should continue to the next sequential statement
            List<CFGNode> outputsToEnd = new ArrayList<>(body.getOutput());
            for (CFGNode out : new ArrayList<>(outputsToEnd)) {
                IASTNode lead = out.getLeadingNode();
                // If this output is an explicit return, keep it going to End via returns list handling
                if (lead instanceof IASTReturnStatement) {
                    continue;
                }
                CFGNode next = findNextCFGNode(lead);
                if (next != null) {
                    // Connect out -> next instead of out -> End
                    this.setOutput(out, next, FlowEdge.class);
                    // remove it from the list that will be connected to End
                    body.getOutput().remove(out);
                }
            }

            // finally connect remaining outputs to End (these are real fall-throughs to function end)
    ...
    public void visit(IASTCompoundStatement pStatement) {
        ...
        if (pStatement.getStatements().length != 0) {
            ...
            this.astToCfgMap.put(pStatement, voidBlock);
    ...
    public void visit(IASTDeclarationStatement pStatement) {
        ...
        this.astToCfgMap.put(pStatement, thisNode);
    ...
    public void visit(IASTIfStatement pStatement) {
        ...
        this.astToCfgMap.put(pStatement.getConditionExpression(), condition);
        ...
        // Map the if statement itself to its condition node so we can locate it in AST->CFG mapping
        this.astToCfgMap.put(pStatement, condition);
    ...
    public void visit(IASTSwitchStatement pStatement) {
        ...
        this.astToCfgMap.put(pStatement.getControllerExpression(), expression);
    ...
    public void visit(IASTWhileStatement pStatement) {
        ...
        this.graph.addVertex(condition);
        this.astToCfgMap.put(pStatement.getCondition(), condition);
    ...
    public void visit(IASTForStatement pStatement) {
        ...
        this.astToCfgMap.put(pStatement.getConditionExpression(), condition);
        this.astToCfgMap.put(pStatement.getIterationExpression(), incr);
    ...
    public void visit(IASTGotoStatement pStatement) {
        ...
        this.astToCfgMap.put(pStatement, node);
    ...
    /**
     * Returns the CFGNode corresponding to the next sequential AST statement after the given AST node.
     * Walks up parent compound statements to find the next sibling statement; returns null if none.
     */
    private CFGNode findNextCFGNode(IASTNode node) {
        ...
        while (parent != null) {
            if (parent instanceof IASTCompoundStatement) {
                ...
                for (int i = 0; i < stmts.length; i++) {
                    if (stmts[i] == node) {
                        // find next sibling
                        for (int j = i + 1; j < stmts.length; j++) {
                            IASTStatement candidate = stmts[j];
                            CFGNode mapped = this.astToCfgMap.get(candidate);
                            if (mapped != null) return mapped;
                        }
                        break; // not found at this level, continue upward
                    }
                }
            }
            // move up to find a parent that contains our node as child
            node = parent;
