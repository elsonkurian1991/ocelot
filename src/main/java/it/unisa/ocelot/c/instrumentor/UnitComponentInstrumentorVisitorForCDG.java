...
public class UnitComponentInstrumentorVisitorForCDG extends ASTVisitor {
	...
	public UnitComponentInstrumentorVisitorForCDG(String pInstrumentFunction, ArrayList<String> testObjectives,
	...
	public IASTExpression transformDistanceExpression(IASTExpression expression, boolean pNegation,
		...
		if (expression instanceof IASTBinaryExpression) {
			...
			/* changing for operators */
				...
				// return this.trasformArithmetic(realExpression, pNegation,
				// IASTBinaryExpression.op_divide);//division was missing before
				return realExpression;
				...
				// return this.trasformArithmetic(realExpression, pNegation,
				// IASTBinaryExpression.op_min);//make changes to handle: if 'and'then min of
				// the distance
			...
			if (realExpression.getOperator() == IASTUnaryExpression.op_not) {
				...
				// CASTCastExpression cast = new CASTCastExpression(type,
				// this.cloneExpression(realExpression.getOperand()));
	...
	public void visit(IASTIfStatement statement) throws Exception {
		// Reserve branch id first so nested branches inside then/else get distinct ids
		int reservedBranch = branchNumber;
		addTestObjectives(reservedBranch);
		branchNumber++;
		statement.getChildren();
		// Build instrumentation using reservedBranch
		...
		instrArgs[1] = new CASTLiteralExpression(CASTLiteralExpression.lk_integer_constant,
				Integer.toString(reservedBranch).toCharArray());
		...
		thenClause.add(functionName + ":" + "branch" + reservedBranch + "-" + "true");
		elseClause.add(functionName + ":" + "branch" + reservedBranch + "-" + "false");

		// Tag the then/else blocks
		...
		// Tag code after the if/else using reservedBranch
		markOutsideIfStatement(statement, reservedBranch);

		if (trackSynthetics.contains(statement)) {
			SyntheticBranches.add(functionName + ":" + "branch" + reservedBranch + "-" + "true");
			SyntheticBranches.add(functionName + ":" + "branch" + reservedBranch + "-" + "false");
			foundSynthetics.add(statement);
		}
	}

	/*
	 // Old visit(IASTIfStatement) implementation (kept for reference)
	 public void visit(IASTIfStatement statement) throws Exception {
	...
	*/
	...
	private void markWhileForDoStatements(IASTStatement statement, IASTNode statementBody, int branchId) throws Exception {
		List<String> Body = new ArrayList<String>();
		List<String> outsideWhile = new ArrayList<String>();
		Body.add(functionName + ":" + "branch" + branchId + "-" + "true");
		outsideWhile.add(functionName + ":" + "branch" + branchId + "-" + "false");
	...
	private void markOutsideIfStatement(IASTStatement statement, int branchId) throws Exception {
		List<String> outsideStatements = new ArrayList<String>();
		outsideStatements.add(functionName + ":" + "branch" + branchId + "-" + "true");
		outsideStatements.add(functionName + ":" + "branch" + branchId + "-" + "false");
	...
	public void visit(IASTWhileStatement statement) throws Exception {
		...
		// Martino
		markWhileForDoStatements(statement, statement.getBody(), reservedBranch);
	...
	/*
	 // Old visit(IASTWhileStatement) implementation (kept for reference)
	 public void visit(IASTWhileStatement statement) throws Exception {
		 IASTExpression[] instrArgs = new IASTExpression[5];
		 instrArgs[0] = new CASTLiteralExpression(CASTLiteralExpression.lk_string_literal,
			 ("\"" + functionName + "\"").toCharArray());
	...
	*/
	...
	/*
	 // Old visit(IASTDoStatement) implementation (kept for reference)
	 public void visit(IASTDoStatement statement) throws Exception {
		 IASTExpression[] instrArgs = new IASTExpression[5];
		 instrArgs[0] = new CASTLiteralExpression(CASTLiteralExpression.lk_string_literal,
			 ("\"" + functionName + "\"").toCharArray());
	...
	*/
	...
	public void visit(IASTForStatement statement) throws Exception {
		...
		// Attempt to extract loop control variable from initializer (e.g., for(i = 0; ...; ...))
		String loopVarName = null;
		IASTStatement initStmt = statement.getInitializerStatement();
		if (initStmt instanceof IASTExpressionStatement) {
			IASTExpression initExpr = ((IASTExpressionStatement) initStmt).getExpression();
			if (initExpr instanceof IASTBinaryExpression) {
				IASTBinaryExpression be = (IASTBinaryExpression) initExpr;
				if (be.getOperator() == IASTBinaryExpression.op_assign) {
					IASTExpression left = be.getOperand1();
					if (left instanceof IASTIdExpression) {
						loopVarName = ((IASTIdExpression) left).getName().toString();
					}
				}
			}
		}
		if (loopVarName == null)
			loopVarName = "forLoopIter_" + reservedBranch;

		// Build ratio expression: loopVar / (loopVar + 1.0)
		IASTIdExpression loopVarId = new CASTIdExpression(new CASTName(loopVarName.toCharArray()));
		IASTLiteralExpression one = new CASTLiteralExpression(IASTLiteralExpression.lk_float_constant, "1.0".toCharArray());
		IASTBinaryExpression denom = new CASTBinaryExpression(IASTBinaryExpression.op_plus, loopVarId.copy(), one);
		IASTBinaryExpression division = new CASTBinaryExpression(IASTBinaryExpression.op_divide, loopVarId.copy(), denom);
		IASTLiteralExpression threshold = new CASTLiteralExpression(IASTLiteralExpression.lk_float_constant, "0.98".toCharArray());
		IASTBinaryExpression ratioGe = new CASTBinaryExpression(IASTBinaryExpression.op_greaterEqual, division.copy(), threshold);

		// Build helper calls _f_ocelot_ge_numeric(division, 0.98) and _f_ocelot_lt_numeric(division, 0.98)
		IASTExpression[] geArgs = new IASTExpression[2];
		geArgs[0] = division.copy();
		geArgs[1] = threshold.copy();
		IASTFunctionCallExpression geCall = makeFunctionCall("_f_ocelot_ge_numeric", geArgs);

		IASTExpression[] ltArgs = new IASTExpression[2];
		ltArgs[0] = division.copy();
		ltArgs[1] = threshold.copy();
		IASTFunctionCallExpression ltCall = makeFunctionCall("_f_ocelot_lt_numeric", ltArgs);

		// Build branch_out call: ("functionName", reservedBranch, ratioGe, geCall, ltCall)
		IASTExpression[] instrArgs = new IASTExpression[5];
		instrArgs[0] = new CASTLiteralExpression(CASTLiteralExpression.lk_string_literal, ("\"" + functionName + "\"").toCharArray());
		instrArgs[1] = new CASTLiteralExpression(CASTLiteralExpression.lk_integer_constant, Integer.toString(reservedBranch).toCharArray());
		instrArgs[2] = ratioGe;
		instrArgs[3] = geCall;
		instrArgs[4] = ltCall;

		IASTFunctionCallExpression branchOutCall = makeFunctionCall("_f_ocelot_branch_out", instrArgs);

		// Preserve original condition and combine: (branchOutCall && originalCondition)
		IASTExpression origCond = statement.getConditionExpression();
		IASTExpression origCondCopy = (origCond != null) ? origCond.copy() : new CASTLiteralExpression(IASTLiteralExpression.lk_integer_constant, new char[] {'1'});
		IASTExpression combined = new CASTBinaryExpression(IASTBinaryExpression.op_logicalAnd, branchOutCall, origCondCopy);

		statement.setConditionExpression(combined);

		// Martino: mark body using reservedBranch
		markWhileForDoStatements(statement, statement.getBody(), reservedBranch);
	}

	/*
	 // Old visit(IASTForStatement) implementation (kept for reference)
	 public void visit(IASTForStatement statement) throws Exception {
		 IASTExpression[] instrArgs = new IASTExpression[5];
		 instrArgs[0] = new CASTLiteralExpression(CASTLiteralExpression.lk_string_literal,
			 ("\"" + functionName + "\"").toCharArray());
	...
	*/
	...
	public int visit(IASTStatement statement) {
		try {
			...
			if (statement instanceof IASTIfStatement)
				...
				// continue traversal (do not skip)
	...
	private IASTExpression transformArithmeticExpression(IASTBinaryExpression pExpression, boolean negation,
		...
		IASTExpression result;
		if (negation) {
			if (pRealOperator == IASTBinaryExpression.op_binaryAnd) {
				result = makeFunctionCall("_f_ocelot_or", operationArgs);
			} else if (pRealOperator == IASTBinaryExpression.op_binaryOr) {
				result = makeFunctionCall("_f_ocelot_and", operationArgs);
			} else {
				System.err.println("Unexpected Binary Expression!");
				result = pExpression;
			}
		} else {
			if (pRealOperator == IASTBinaryExpression.op_binaryAnd) {
